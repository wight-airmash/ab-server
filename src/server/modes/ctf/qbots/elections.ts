...
export default class Elections extends System {
  ...
  onStartElections(initiatorId: PlayerId): void {
    ...
    if (
      ...
      (initiator.team.current === CTF_TEAMS.RED && this.leaders.isRedElections === true)
    ) {
      this.emit(
        RESPONSE_COMMAND_REPLY,
        connectionId,
    ...
    if (initiator.team.current === CTF_TEAMS.BLUE) {
      ...
      if (this.isPlayerAFK(this.leaders.blueId, now) === true) {
      ...
      if (this.isPlayerAFK(this.leaders.redId, now) === true) {
