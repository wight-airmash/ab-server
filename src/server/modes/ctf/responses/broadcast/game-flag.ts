...
import { BROADCAST_GAME_FLAG, CONNECTIONS_SEND_PACKET } from '@/events';
...
export default class GameFlagBroadcast extends System {
  ...
  onGameFlag(flagTeam: CTF_TEAMS, playerId: PlayerId = null): void {
    ...
    if (flagTeam === CTF_TEAMS.BLUE) {
      flag = this.storage.mobList.get(this.storage.ctfFlagBlueId);
    } else {
      flag = this.storage.mobList.get(this.storage.ctfFlagRedId);
    }

    this.log.debug('BROADCAST_GAME_FLAG', {
